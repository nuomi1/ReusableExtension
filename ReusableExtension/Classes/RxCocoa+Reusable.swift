...
public extension Reactive where Base: UICollectionView {
    func items<S: Sequence, T: UICollectionViewCell, O: ObservableType>(cellType: T.Type = T.self)
        -> (_ source: O)
...
public extension Reactive where Base: UITableView {
    func items<S: Sequence, T: UITableViewCell, O: ObservableType>(cellType: T.Type = T.self)
        -> (_ source: O)
        -> (_ configureCell: @escaping (Int, S.Iterator.Element, T) -> Void)
        -> Disposable where T: Reusable, O.E == S {
        return items(cellIdentifier: cellType.reuseIdentifier, cellType: cellType)
